...
def train_val_test_split_adjacency(A, p_val=0.10, p_test=0.05, seed=0, neg_mul=1,
    ...
    # assert not np.any(A.sum(0).A1 + A.sum(1).A1 == 0)  # no dangling nodes (every node must appear once)
    ...
    if asserts:
        ...
        # print(len(set(map(tuple, test_zeros))), len(test_ones))
        # input(0)
        assert len(set(map(tuple, test_zeros))) == len(test_ones) * neg_mul
        # print(set(map(tuple, val_zeros)))
        # print(val_zeros)
        # print(len(set(map(tuple, val_zeros))))
        # print(len(val_ones) * neg_mul)
        # input()

        print("***diff between val zeros and val ones: ", len(set(map(tuple, val_zeros))), len(val_ones))
        # input()
        # assert len(set(map(tuple, val_zeros))) == len(val_ones) * neg_mul # ori
        # assert len(set(map(tuple, val_zeros))) == len(val_ones) * neg_mul - 10 # fix
...
def random_walk(edges, node_ixs, rwlen, p=1, q=1, n_walks=1):
    ...
    for w in range(n_walks):
        ｓource_node = np.random.choice(N)
        #　source_node = w
        walk.append(source_node)
        for it in range(rwlen-1):
            
            if walk[-1] == N-1: # prevent select the last node
...
class RandomWalker:
    ...
    def __init__(self, adj, rw_len, p=1, q=1, batch_size=128):
        ...
        #     input()
        # print("--------------------------------------------")
        # print(np.unique(self.edges[:, 0], return_index=True))
        # print(np.unique(self.edges[:, 0], return_index=True)[0])
        # print(np.unique(self.edges[:, 0], return_index=True)[1])
        # print("--------------------------------------------")
        # print(self.node_ixs)
        # input()
    ...
    def walk(self):
        while True:
            # print(random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size))
            # print(type(random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size)))
            # print(random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size).shape)
            # input()
            # print(random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size).reshape([-1, self.rw_len]))
            # input("input walker")
            yield random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size).reshape([-1, self.rw_len])
            # return random_walk(self.edges, self.node_ixs, self.rw_len, self.p, self.q, self.batch_size).reshape([-1, self.rw_len])
...
def graph_from_scores(scores, n_edges):
    ...
    """
    # print(len(scores.nonzero()[0]), " ", n_edges)
    ...
    for n in np.random.choice(N, replace=False, size=N): # Iterate the nodes in random order
        ...
        # target = np.random.choice(N, p=probs) # follow prob vector to smaple
        target = np.argmax(probs) # choose max value
    ...
    if diff > 0:
        ...
        triu[target_g > 0] = 0 # target_g選到的不再選
        ...
        extra_edges = triu[triu_ixs].argsort()[-int(diff):]
        # print(triu[triu_ixs])
...
def young_graph_from_scores(scores, n_edges):
    '''
    young fixed version
    '''
    if len(scores.nonzero()[0]) < n_edges:
    ...
    target_g = np.zeros(scores.shape) # initialize target graph
    temp_g = np.zeros(scores.shape) # temp target graph, for further downsize
    scores_int = scores.toarray().copy() # internal copy of the scores matrix
    scores_int[np.diag_indices_from(scores_int)] = 0  # set diagonal to zero
    degrees_int = scores_int.sum(0)   # The row sum over the scores.
    ...
    for n in np.random.choice(N, replace=False, size=N): # Iterate the nodes in random order
        ...
        temp_g[n, target] = probs[target]
        temp_g[target, n] = probs[target]


    diff = int((target_g.sum() - n_edges)/2)
    if diff < 0:
        '''
        current number of edges fewer than desired size,
        so, choose more edge by the value in each entry from high to low
        '''
        triu = np.triu(scores_int)
        triu[target_g > 0] = 0 # target_g選到的不再選
        triu[np.diag_indices_from(scores_int)] = 0
        ...
        return symmetric(target_g)

    else:
        '''
        cause the target g didn't satisfy the desired size,
        so, need further downsize

        use temp g as scores, and choose the edge by the value in each entry 
        from high to low, until this graph satisfy desired size
        
        '''
        target_g = np.zeros(scores.shape) # reset the target g
        scores_int = temp_g.copy() # use temp_g as scores
        scores_int[np.diag_indices_from(scores_int)] = 0  # set diagonal to zero
        triu = np.triu(scores_int)
        triu = triu / triu.sum()
        triu_ixs = np.triu_indices_from(scores_int)

        select_edges = triu[triu_ixs].argsort()[-int(n_edges/2):]
        target_g[(triu_ixs[0][select_edges], triu_ixs[1][select_edges])] = 1
        target_g[(triu_ixs[1][select_edges], triu_ixs[0][select_edges])] = 1

        return symmetric(target_g)
        


def arg_graph_from_scores(scores, n_edges):
    # print(scores)
    # print(type(scores))
    # print(scores.nonzero())
    # print(scores.nonzero()[0])
    # input()
...
def is_k_anonymous(df, partition, k=50):
    if len(partition) < k:
        return False
    return True

def partition_dataset(df, feature_columns, scale, is_valid):
    # input("in partition dataset")
    finished_partitions = []
    partitions = [df.index]
    while partitions:
